...
# regex
dictfile = open("/appsec/jsscanner/whxxxjr.one/r.backup", "r")
dictcontent  = dictfile.read()
_regex = ast.literal_eval(dictcontent)
dictfile.close()
...
def estimate_shannon_entropy(rawstring):
    m = len(rawstring)
    bases = collections.Counter([tmp_base for tmp_base in rawstring])
...
for string in strings:
        string= string.strip()
        if estimate_shannon_entropy(string) >4.2:
        #print(string)
                print('High Entropy Found' + '\t->\t' +str(string) + " -  " + str(round(estimate_shannon_entropy(string.strip()), 2)))
